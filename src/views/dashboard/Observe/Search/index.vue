<template>
  <el-card>
    <div slot="header" class="header">
      <div class="search-header">
        <span>热门搜索</span>
        <el-dropdown>
          <span>
            <i class="el-icon-more" />
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item>鼠标</el-dropdown-item>
            <el-dropdown-item>键盘</el-dropdown-item>
            <el-dropdown-item>山地车</el-dropdown-item>
            <el-dropdown-item>化妆品</el-dropdown-item>
            <el-dropdown-item>文具</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
    </div>
    <div>
      <el-row :gutter="10">
        <el-col :span="12">
          <LineCharts />
        </el-col>
        <el-col :span="12">
          <LineCharts />
        </el-col>
      </el-row>
      <!-- table表格 -->
      <el-table :data="tableData" style="width: 100%" border>
        <el-table-column label="排名" width="80" type="index" />
        <el-table-column label="搜索关键字" width="180" />
        <el-table-column label="用户数" sortable />
        <el-table-column label="周涨幅" sortable />
      </el-table>
      <!-- 分页去 -->
      <el-pagination layout="prev, pager, next" :total="100" class="pagination" />
    </div>
  </el-card>
</template>

<script>
import LineCharts from './lineCharts'
export default {
  name: '',
  components: {
    LineCharts
  },
  data() {
    return {
      tableData: [{}]
    }
  }
}
</script>

<style scoped>
.header {
  border-bottom: 1px solid #eee;
  padding: 5px 0px;
}
.search-header {
  display: flex;
  justify-content: space-between;
}
.pagination {
  float: right;
}
</style>
